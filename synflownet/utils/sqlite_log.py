...
class SQLiteLogHook:
    ...
    def __call__(self, trajs, rewards, obj_props, cond_info):
        ...
        data = [
            [objs[i], rewards[i], trajs[i]] + obj_props[i] + preferences[i] + focus_dir[i] + [cond_info[k][i].item() for k in logged_keys]
...
class SQLiteLog:
    ...
    def connect(self, db_path: str):
        ...
        cur = self.db.cursor()
        self._has_results_table = len(cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='results'").fetchall())
        cur.close()
    ...
    def insert_many(self, rows, column_names):
        assert all([isinstance(x, str) or not isinstance(x, Iterable) for x in rows[0]]), "rows must only contain scalars"
