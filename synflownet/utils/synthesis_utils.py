...
def run_reaction_with_fallback(rxn, product):
    ...
    if len(rs) == 0:
        ...
        rs = rxn.RunReactants((product,))
    if len(rs) == 0:
        logger.debug(f"Reaction did not yield any products. Product: {Chem.MolToSmiles(product)}")
    return rs
...
class Reaction:
    def __init__(self, template=None, building_blocks=None):
    ...
    def run_reactants(
        self,
        reactants: tuple[Union[Chem.Mol, str, None]],
        rxn: rdChemReactions = None,
        keep_main: bool = True,
    ) -> Union[Chem.Mol, None]:
        ...
        if len(ps) == 0:
            ...
            for i in range(len(ps)):
                ...
                p = ps[i][0]
                p_canon = Chem.MolFromSmiles(Chem.MolToSmiles(p, canonical=True))
            ...
            if len(rs) == 1:
                ...
                for i, reactant_set in enumerate(rs):
                    ...
                    if all(mol is not None for mol in rs_canon):
                        ...
                        if any(smi in self.building_blocks for smi in reactant_smiles):
                            return rs_canon
                        for r_i in reactant_list[2:]:
                            if Chem.MolToSmiles(r_i) in self.building_blocks:
...
def get_mol_embeddings(smiles: list[str], fp_type: Optional[str], fp_path: Optional[str]) -> torch.Tensor:
    ...
    if fp_type == "precomputed":
        assert fp_path is not None, "Path to precomputed embeddings must be provided."
        embeddings = torch.load(fp_path)
        return embeddings
    if fp_type == "molgps":
