...
class FocusRegionConditional(Conditional[tuple[ObjectProperties, LogScalar], LogScalar]):
    ...
    def sample(self, n: int, train_it: Optional[int] = None):
        ...
        if self.fixed_focus_dirs is not None:
            focus_dir = torch.tensor(np.array(self.fixed_focus_dirs)[rng.choice(len(self.fixed_focus_dirs), n)].astype(np.float32))
            ...
            focus_dir = torch.tensor(metrics.sample_positiveQuadrant_ndim_sphere(n, self.n_objectives, normalisation="l2")).float()
        elif self.cfg.focus_type is not None and "learned" in self.cfg.focus_type:
            if self.focus_model is not None and train_it >= self.cfg.focus_model_training_limits[0] * self.cfg.max_train_it:
