...
class MultiObjectiveStatsHook:
    ...
    def _run_pareto_accumulation(self):
        ...
        while not self.stop.is_set() and timeouts < 200:
            try:
                ...
                logger.debug("Pareto Accumulation thread Queue ConnectionError", e)
                break
            ...
            if num_updates % self.save_every == 0:
                if self.pareto_queue.qsize() > 10:
                    logger.warning("Warning: pareto metrics computation lagging")
                self._save()
        self._save()
    ...
    def __call__(self, trajs, rewards, flat_rewards, cond_info):
        ...
        if self.compute_focus_accuracy:
            focus_acc = metrics.get_focus_accuracy(torch.tensor(flat_rewards), torch.tensor(focus_dirs), self.focus_cosim)
            info = {
                **info,
                "focus_acc": focus_acc,
            }
