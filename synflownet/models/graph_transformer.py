...
def mlp(n_in=1, n_hid=1, n_out=1, n_layer=0, act=nn.LeakyReLU, ns=None):
    ...
    """
    n = ([n_in] + [n_hid] * n_layer + [n_out]) if ns is None else ns
    n_layer = len(n) - 2  # In case ns is provided, override n_layer
...
class GraphTransformerGFN(nn.Module):
    ...
    action_type_to_key = (  # noqa: E731
        lambda action_type: GraphTransformerGFN._graph_part_to_key.get(  # noqa: E731
            GraphTransformerGFN._action_type_to_graph_part.get(action_type)  # noqa: E731
        )  # noqa: E731
    )
    ...
    def __init__(
        ...
        num_emb = cfg.model.num_emb
        num_mlp_emb = int(num_emb * cfg.model.mlp_width)
        num_final = num_emb
        ...
        init_with_fps = cfg.model.graph_transformer.init_with_fps
        if init_with_fps:
            fps = torch.load(cfg.model.graph_transformer.fingerprint_path)
        ...
        building_blocks_dim = env_ctx.building_blocks_dim if self.continuous_action_embs else env_ctx.num_building_blocks
        ...
        mlps = {}
        n_layers = cfg.model.graph_transformer.num_mlp_layers
        for atype in chain(env_ctx.action_type_order, env_ctx.bck_action_type_order if do_bck else []):
            ...
            if init_with_fps and atype in [
                GraphActionType.AddFirstReactant,
                GraphActionType.AddReactant,
            ]:
                mlps[atype.cname] = mlp(ns=[num_in] + [num_mlp_emb] * max(0, (n_layers - 1)) + list(fps.T.shape))
                mlps[atype.cname][-1].weight.data = fps
            else:
                mlps[atype.cname] = mlp(num_in, num_mlp_emb, num_out, n_layers)
        ...
        self._logZ = mlp(max(1, env_ctx.num_cond_dim), num_mlp_emb, 1, 2)
    ...
    def call_mlp(self, mlp_name, input):
        ...
        if self.continuous_action_embs and mlp_name == GraphActionType.AddReactant.cname:
            ...
            mlp_outputs = torch.matmul(mlp_outputs, self.env_ctx.building_blocks_embs.T)
        return mlp_outputs
...
class GraphTransformerSynGFN(GraphTransformerGFN):
    ...
    def _make_cat(
        ...
        g: gd.Batch,
        emb: dict[str, Tensor],
        action_types: list[GraphActionType],
        fwd: bool,
    ):
        raw_logits = []
        for t in action_types:
            mlp_outputs = self.mlps[t.cname](emb[self._action_type_to_graph_part[t]])
            if self.continuous_action_embs and t in [
                GraphActionType.AddFirstReactant,
                GraphActionType.AddReactant,
            ]:
                # For continuous action embeddings, we obtain logits by computing dot product between MLP output and embeddings
    ...
    def forward(self, g: gd.Batch, cond: Optional[torch.Tensor]):
        ...
        if self.do_bck:
            bck_cat = self._make_cat(g, emb, self.bck_action_type_order, fwd=False)
            return fwd_cat, bck_cat, graph_out
        return fwd_cat, graph_out
