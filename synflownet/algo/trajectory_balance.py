...
class TrajectoryBalance(GFNAlgorithm):
    ...
    def __init__(
        ...
        assert (
            self.cfg.backward_policy not in [Backward.MaxLikelihood, Backward.REINFORCE, Backward.Free] or self.cfg.do_parameterize_p_b
        ...
        for traj in trajs:
            n_back = [
                self.env.count_backward_transitions(
                    gp,
                    traj_len=len(traj["traj"]),
                    check_idempotent=self.cfg.do_correct_idempotent,
                )
    ...
    def construct_batch(self, trajs, cond_info, log_rewards):
        ...
        if self.model_is_autoregressive:
            ...
            actions = [self.ctx.GraphAction_to_ActionIndex(g, i[1], fwd=True) for g, tj in zip(torch_graphs, trajs) for i in tj["traj"]]
        else:
        ...
        if self.cfg.do_parameterize_p_b:
            batch.bck_actions = [
                self.ctx.GraphAction_to_ActionIndex(g, a, fwd=False) for g, a in zip(torch_graphs, [i for tj in trajs for i in tj["bck_a"]])
            ]
        ...
        if self.cfg.do_correct_idempotent:
            ...
            ipa = [self.get_idempotent_actions(g, gd, gp, a) for g, gd, gp, a in zip(agraphs, torch_graphs, bgraphs, gactions)]
            ...
            if self.cfg.do_parameterize_p_b:
                ...
                bck_ipa = [self.get_idempotent_actions(g, gd, gp, a) for g, gd, gp, a in zip(agraphs, torch_graphs, bgraphs, gactions)]
    ...
    def compute_batch_losses(
        ...
        if self.cfg.do_parameterize_p_b:
            ...
            if logger.level == "DEBUG":
                fwd_is_stuck, n_fwd_stuck, n_fwd_batch = fwd_cat.check_if_stuck()
                if fwd_is_stuck:
                    logger.debug(
                        f"Forward is stuck: {int(n_fwd_stuck)}/{int(n_fwd_batch)} elements of the batch with no forward action allowed"
                    )
                bwd_is_stuck, n_bwd_stuck, n_fwd_batch = bck_cat.check_if_stuck()
                if bwd_is_stuck:
                    logger.debug(
                        f"Backward is stuck: {int(n_bwd_stuck)}/{int(n_fwd_batch)} elements of the batch with no backward action allowed"
                    )
        else:
            if self.model_is_autoregressive:
        ...
        if self.cfg.do_correct_idempotent:
            ...
            if self.cfg.do_parameterize_p_b:
                ...
                bck_ip_batch_idces = torch.arange(batch.bck_ip_lens.shape[0], device=dev).repeat_interleave(batch.bck_ip_lens)
        ...
        if self.cfg.do_parameterize_p_b:
            ...
            for i, val in enumerate(log_p_B):
                if val <= -900:
                    raise ValueError(
                        f"Invalid log_p_B: {val}, {i}, {batch.bck_actions[i + 1]}, "
                        f"{Chem.MolToSmiles(self.ctx.graph_to_obj(batch.nx_graphs[i + 1]))}"
                    )
        else:
        ...
        info = {
            ...
            "invalid_trajectories": (invalid_mask[batch.from_p_b == 0].sum() / batch.num_online if batch.num_online > 0 else 0),
            ...
            "pF_entropy": fwd_cat.entropy().mean(),
    ...
    def _init_subtb(self, dev):
        ...
        self._precomp = [
            (
                ...
                torch.cat([ar[: T - i].repeat_interleave(ar[: T - i] + 1) + ar[T - i + 1 : T + 1].sum() for i in range(T)]),
            )
