...
class SoftQLearning:
    ...
    def create_training_data_from_own_samples(self, model: nn.Module, n: int, cond_info: Tensor, random_action_prob: float):
        ...
        data = self.graph_sampler.sample_from_model(model, n, cond_info, random_action_prob, use_argmax=self.alpha == 0.0)
        return data
    ...
    def construct_batch(self, trajs, cond_info, log_rewards):
        ...
        actions = [
            self.ctx.GraphAction_to_ActionIndex(g, a, fwd=True) for g, a in zip(torch_graphs, [i[1] for tj in trajs for i in tj["traj"]])
        ]
    ...
    def _update_lagged_model(self, model):
        ...
        for key in policy_net_state_dict:
            target_net_state_dict[key] = (
                self.lagged_model_tau * policy_net_state_dict[key] + (1 - self.lagged_model_tau) * target_net_state_dict[key]
            )
    ...
    def compute_batch_losses(self, model: nn.Module, batch: gd.Batch, num_bootstrap: int = 0):
        ...
        with torch.no_grad():
            if self.alpha < 0.0:
                ...
                next_step_value = Q_tilde_lagged.logsumexp(actions=batch.actions, nx_graphs=batch.nx_graphs, model=model).detach()
            elif self.alpha == 0.0:
                ...
                max_secondary = torch.nan_to_num(max_secondary, nan=-float("inf"))
            ...
            step_rewards[final_graph_idx] = rewards + (1 - batch.is_valid) * self.invalid_penalty  # we only have a terminal reward
