...
class SynthesisSampler(Sampler):
    ...
    def sample_from_model(
        ...
        cond_info: Tensor
            ...
            if None, a tensor of zeros is used
        ...
        if cond_info is None:
            cond_info = torch.zeros((n, max(1, self.ctx.num_cond_dim)), device=get_worker_device())
            ...
            for i, j in zip(not_done(range(n)), range(n)):
                ...
                if graph_actions[j].action is GraphActionType.Stop:
                    ...
                    try:
                        ...
                        if strict_forward_policy:
                            ...
                            if Chem.MolToSmiles(bck_mol) != Chem.MolToSmiles(self.ctx.graph_to_obj(graphs[i])):
                                if both_are_bb:
                                    if self.ctx.building_blocks[bb_idx] != Chem.MolToSmiles(self.ctx.graph_to_obj(graphs[i])):
                                        raise ValueError("Reversing action does not yield the original molecule.")
                                else:
                                    raise ValueError("Reversing action does not yield the original molecule.")
                        bck_a[i].append(b_a)
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Issue when sanitizing molecule {Chem.MolToSmiles(gp)}: {e}."
                            f"\n Action: {graph_actions[j]}, "
                            f"Original mol: {Chem.MolToSmiles(self.ctx.graph_to_obj(graphs[i]))}"
                        )
                    ...
                    if not self.cfg.algo.tb.do_parameterize_p_b:
                        n_back = self.env.count_backward_transitions(
                            g,
                            traj_len=len(data[i]["traj"]),
                            check_idempotent=self.correct_idempotent,
                        )
                        if n_back > 0:
        ...
        logger.debug(f"Number of reactions that could not be reversed: {could_not_reverse}")
        return data
    ...
    def sample_backward_from_graphs(
        ...
        if random_action_prob > 0:
            pass  # TOOD: implement random action sampling
        ...
        while sum(done) < n and t < self.max_len:
            ...
            masks_sum = [bck_react_uni_masks[i] + bck_react_bi_masks[i] + remove_masks[i] for i in range(len(torch_graphs))]
            not_done_mask = torch.tensor(done, device=dev).logical_not()
            ...
            graph_bck_actions = [self.ctx.ActionIndex_to_GraphAction(g, a, fwd=False) for g, a in zip(torch_graphs, bck_actions)]
                ...
                if not done[i]:
                    ...
                    try:
                        ...
                        if b_a.action == GraphActionType.BckRemoveFirstReactant:
                            data[i]["bbs"].append(self.ctx.building_blocks.index(Chem.MolToSmiles(self.ctx.graph_to_obj(g))))
        ...
        for i in range(n):
            if data[i]["bck_a"][-1].action != GraphActionType.BckRemoveFirstReactant or not data[i]["is_valid_bck"]:
