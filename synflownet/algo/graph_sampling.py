...
class GraphSampler(Sampler):
    ...
    def sample_from_model(
        ...
        # Note: instead of padding with Stop, we could have a virtual action whose probability
        ...
        for t in range(self.max_len):
            ...
            if random_action_prob > 0:
                ...
                is_random_action = torch.tensor(rng.uniform(size=len(torch_graphs)) < random_action_prob, device=dev).float()
        ...
        for i in range(n):
            ...
            if self.pad_with_terminal_state:
                # Note: instead of padding with Stop, we could have a virtual action whose
