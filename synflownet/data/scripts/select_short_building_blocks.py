...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--filename",
        type=str,
        help="Path to input building blocks",
        default="building_blocks.txt",
    )
    parser.add_argument(
        "--output_filename",
        type=str,
        help="Path to output building blocks",
        default="short_building_blocks.txt",
    )
    parser.add_argument("--num_atoms", type=int, help="Maximum number of atoms", default=20)
    args = parser.parse_args()

    with open(Path(__file__).parent / args.filename) as file:
