...
class ReplayConfig(StrictDataClass):
    ...
    halt_training_when_queue_is_full : bool
        When using async replay buffer: If True, halt training when reward queue is full. If False, use FIFO behavior
        to overwrite oldest entries when queue is full and continue training.
    """
    ...
    buffer_is_async: bool = False
    reward_queue_path: Optional[str] = None
    reward_queue_max_size: Optional[int] = None
    persistent_replay_path: Optional[str] = None
    persistent_replay_max_size: Optional[int] = None
    persistent_replay_last_to_random_ratio: Optional[float] = None
    halt_training_when_queue_is_full: bool = False
