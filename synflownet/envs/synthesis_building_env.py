...
class ReactionTemplateEnvContext(GraphBuildingEnvContext):
    ...
    def __init__(
        ...
        rev_reaction_templates: Optional[list[str]] = None,
        building_blocks: list[str] = None,
        precomputed_bb_masks: np.ndarray = None,
        fp_type: str = None,
        fp_path: str = None,
        add_hs: bool = False,  # NOTE: Needs to be true if using REAL reactions
        building_blocks_costs: Optional[dict] = None,
        ...
        self.reactions = [Reaction(template=t, building_blocks=building_blocks) for t in reaction_templates]  # Reaction objects
        ...
        if rev_reaction_templates is not None:
            self.reverse_reactions = [Reaction(template=t, building_blocks=building_blocks) for t in rev_reaction_templates]
        ...
        self.building_blocks_embs = get_mol_embeddings(building_blocks, fp_type=fp_type, fp_path=fp_path).to(
            device
        )  # send bb_embs to device now to avoid multiple references in forward pass
        self.building_blocks_dim = self.building_blocks_embs.shape[1]
        if ReactionTaskConfig.sanitize_building_blocks:
            logger.info("Sanitizing building blocks ...")
            building_blocks_mols = [Chem.MolFromSmiles(bb) for bb in building_blocks]
            building_blocks_sanitized = []
            remover = SaltRemover.SaltRemover()  # some molecules are salts, we want the sanitized version of them not to have the salt
            for bb in building_blocks_mols:
                try:
                    ...
                    logger.debug(f"Failed to sanitize building block {Chem.MolToSmiles(bb)}: {e}")
        ...
        if precomputed_bb_masks is None:
            self.precomputed_bb_masks = np.zeros((2, self.num_bimolecular_rxns, self.num_building_blocks))
        else:
    ...
    def ActionIndex_to_GraphAction(self, g: gd.Data, aidx: ActionIndex, fwd: bool = True) -> GraphAction:
        ...
        -------
        action: GraphAction
            An action whose type is one of Stop, ReactUni, ReactBi, AddReactant,
            AddFirstReactant, BckReactUni, BckReactBi, BckRemoveFirstReactant.
        """
        if fwd:
            action_type_order = self.action_type_order
        else:
    ...
    def GraphAction_to_ActionIndex(self, g: gd.Data, action: GraphAction, fwd: bool = True) -> int:
        ...
        ----------
        action: GraphAction
            An action whose type is one of Stop, ReactUni, ReactBi, AddReactant, AddFirstReactant,
            BckReactUni, BckReactBi, BckRemoveFirstReactant.
        fwd: bool, default=True
            Whether the action is a forward or backward action.
    ...
    def create_masks(self, smi: Union[str, Chem.Mol, Graph], action_type: GraphActionType, traj_len: int) -> Union[list[int], Any]:  # type: ignore
        ...
        if (
            ...
            return np.zeros(masks_len[action_type])  # we don't want to stop at t=1 and just return a BB
        elif traj_len > 1 and action_type == GraphActionType.Stop:
            return np.ones(masks_len[action_type])
        elif action_type == GraphActionType.BckRemoveFirstReactant:
            mol = self.get_mol(smi)  # type: ignore
            if self.add_hs:
                # We use ctx.add_hs to add hydrogens to the building blocks when in RDMol format
                # to accomodate some reaction templates that explicitly specify hydrogens.
                # However, we assume that the building block SMILES list does not specify hydrogens
                # and must remove them before checking if the current mol is in the building block set.
            ...
            sanitized_mol = Chem.Mol(mol)  # type: ignore
            sanitized_mol = Chem.MolFromSmiles(Chem.MolToSmiles(sanitized_mol))  # type: ignore
            Chem.Kekulize(sanitized_mol, clearAromaticFlags=True)  # type: ignore
            if self.add_hs:
                mol = Chem.AddHs(mol)
            ...
            rxn_possible_mask = self.check_if_any_bb_can_react(mol) if action_type == GraphActionType.ReactBi else None
            for idx, rxn in enumerate(reactions):
                if action_type is GraphActionType.ReactUni:
                    if not rxn.is_reactant(mol):
                        masks[idx] = 0
                elif action_type is GraphActionType.ReactBi:
                    if not rxn.is_reactant(mol):
                        masks[idx] = 0
                    else:
                        masks[idx] = rxn_possible_mask[idx]
                else:
                    assert action_type in [
                        GraphActionType.BckReactUni,
                        GraphActionType.BckReactBi,
                    ...
                    if self.reverse_reactions is not None:
                        if not (rxn.is_reactant(mol) or rxn.is_reactant(sanitized_mol)):
                            masks[idx] = 0
                    else:
                        if not (rxn.is_product(mol) or rxn.is_product(sanitized_mol)):
                            masks[idx] = 0

                    if self.strict_bck_masking and action_type == GraphActionType.BckReactBi:
                        # first we checked if the reactions matched, now we check if at least one of the products is a building block
                        if masks[idx] == 1:
                            ps = rxn.run_reverse_reactants((mol,))
                            if self.add_hs:
                                # We assume the building block set does not have explicit hydrogens even if the rxn templates do
                                ps = [Chem.RemoveHs(p) for p in ps]  # type: ignore
    ...
    def create_masks_for_bb(self, smi: Union[str, Chem.Mol, Graph], bimolecular_row_idx: int) -> list[bool]:  # type: ignore
        raise NotImplementedError(
            "This function has been replaced by `synflownet.envs.ReactionTemplateEnvContext.create_masks_for_bb_from_precomputed()`"
            " and `synflownet.data.scripts.precompute_bb_masks.compute_bb_masks()`."
        )

    def create_masks_for_bb_from_precomputed(self, smi: Union[str, Chem.Mol, Graph], bimolecular_row_idx: int) -> list[bool]:
    ...
    def check_if_any_bb_can_react(self, smi: Union[str, Chem.Mol, Graph]) -> list[bool]:
        # We want to return an array of bools, one for each reaction, that says whether any bb can react with the current mol
        # We first build a mask for the current mol, by flipping the reactants ordering
        mol = self.get_mol(smi)
        mol_matches = np.array(
            [
                [
                    mol.HasSubstructMatch(reactants[1]),
                    mol.HasSubstructMatch(reactants[0]),
                ]
                for reaction in self.bimolecular_reactions
                for reactants in [reaction.rxn.GetReactants()]
            ]
        ).T
        # AND-operator between mol_matches and precomputed_any_bb_reacts give us, for both reactant ordering i.e. (mol, bb) and (bb, mol)
        # whether we have a fit. Any of these 2 fits can work.
        return np.any(np.logical_and(self.precomputed_any_bb_reacts, mol_matches), axis=0).astype(np.float64)
    ...
    def obj_to_graph(self, mol: Chem.Mol) -> Graph:
        ...
        if self.add_hs:
            # We add explicit hydrogens to accommodate some reaction templates that explicitly specify hydrogens
            # However, for the model (graph transformer) we do not want to include hydrogens in the graph (unnecessary big graphs)
            mol = Chem.RemoveHs(mol)
        try:
            ...
            logger.debug(f"Failed to update property cache for mol {Chem.MolToSmiles(mol)}: {e}")
    ...
    def graph_to_obj(self, g: Graph) -> Chem.Mol:
        ...
        try:
            ...
            mp = Chem.Mol(rw_mol)
        Chem.SanitizeMol(mp)
        mp = Chem.MolFromSmiles(Chem.MolToSmiles(mp))
        if self.add_hs:
            mp = Chem.AddHs(mp)
        return mp
    ...
    def graph_to_Data(self, g: Graph, traj_len: int) -> gd.Data:
        ...
        bck_remove_first_reactant_mask = self.create_masks(g, action_type=GraphActionType.BckRemoveFirstReactant, traj_len=traj_len)
        ...
        data = dict(
            ...
            bck_remove_first_reactant_mask=bck_remove_first_reactant_mask.reshape((1, len(bck_remove_first_reactant_mask))),
        )
...
class ReactionTemplateEnv:
    ...
    def __init__(
        ...
        assert isinstance(ctx, ReactionTemplateEnvContext)
        self.ctx = ctx
    ...
    def step(self, smi: Union[str, Chem.Mol, Graph], action: GraphAction) -> Chem.Mol:
        ...
        if action.action is GraphActionType.Stop:
            return mol
        elif action.action is GraphActionType.AddFirstReactant:
            ...
            raise ValueError(f"Unrecognized action type: {action.action}")
    ...
    def backward_step(self, smi: Union[str, Chem.Mol, Graph], action: GraphAction) -> Chem.Mol:
        ...
        if action.action is GraphActionType.BckRemoveFirstReactant:
            ...
            if self.ctx.add_hs:
                # We assume the building block set does not have explicit hydrogens even if the rxn templates do
                products = [Chem.RemoveHs(p) for p in products]
            ...
            if (products_smi[0] in all_bbs) and (products_smi[1] in all_bbs):
                ...
                raise ValueError(f"Neither product is a building block. Products: {products_smi}, reaction: {reaction.template}")
            ...
            for idx in sorted(atoms_to_remove, reverse=True):  # Remove atoms in reverse order to avoid reindexing issues
                rw_mol.ReplaceAtom(idx, Chem.Atom("H"))
            atoms_to_remove = [atom.GetIdx() for atom in rw_mol.GetAtoms() if atom.GetSymbol() in ["[CH]", "[C@@H]", "[C@H]"]]
            for idx in sorted(atoms_to_remove, reverse=True):  # Remove atoms in reverse order to avoid reindexing issues
                rw_mol.ReplaceAtom(idx, Chem.Atom("C"))
            try:
                rw_mol.UpdatePropertyCache()
            except Chem.rdchem.AtomValenceException as e:
                logger.warning(f"{e}: Reaction {reaction.template}, product {Chem.MolToSmiles(selected_product)}")
    ...
    def parents_count(self, g: Graph, traj_len: int) -> int:
        ...
        """
        parents_count = 0
        gd = self.ctx.graph_to_Data(g, traj_len=traj_len)
    ...
    def parents_count_idempotent(self, g: Graph, traj_len: int) -> int:
        """Returns the number of parent molecules of a given molecule.
        ...
        for tidx, atype in enumerate(self.ctx.bck_action_type_order):
            ...
            for o in nza:
                ...
                if both_are_bb:
                    for s_p in [
                        sp,
                        Chem.MolFromSmiles(self.ctx.building_blocks[bb_idx]),
                    ]:
    ...
    def count_backward_transitions(self, g: Graph, traj_len: int, check_idempotent: bool = False) -> int:
        ...
        p_c = self.parents_count(g, traj_len)
        if check_idempotent:
            p_c_2 = self.parents_count_idempotent(g, traj_len)
            if p_c != p_c_2:
                logger.debug(f"Number of parents: {p_c}. Idempotent: {p_c_2}.")
            return p_c_2
        return p_c
...
class ActionCategorical:
    ...
    def instantiate_action_hierarchy(self, types: list[GraphActionType]):
        """
        For fwd actions, there is a hierarchy of action types:
            AddFirstReactant, Stop, UniReact, BiReact to be sampled first, then AddReactant
        ...
        if self.fwd:
            self.action_type_to_logits_index = {action_type: i for i, action_type in enumerate(types + [GraphActionType.AddReactant])}
        else:
    ...
    def instantiate_secondary_logits(self):
        ...
        if "secondary" in self.action_hierarchy[key]:
            ...
            secondary_action_idxs = [self.action_type_to_logits_index.get(action_type) for action_type in secondary_action_types]
        ...
        self.raw_logits.append(
            torch.full(
                size=(self.num_graphs, self.ctx.num_building_blocks),
        ...
        self._action_masks.append(
            torch.ones(
                size=(self.num_graphs, self.ctx.num_building_blocks),
                dtype=self._action_masks[0].dtype,
                device=self.dev,
            )
        )
    ...
    def _compute_batchwise_max(
        ...
        if do_entropy:
            primary_logits = [torch.where(logit == -float("inf"), torch.tensor(-1000.0), logit) for logit in primary_logits]
        ...
        if secondary_logits:
            ...
            # assert do_entropy or not torch.isinf(
    ...
    def logsoftmax(self, do_entropy: Optional[bool] = False):
        ...
        if do_entropy:
            primary_logits = [torch.where(logit == -float("inf"), torch.tensor(-1000.0), logit) for logit in primary_logits]
            secondary_logits = [torch.where(logit == -float("inf"), torch.tensor(-1000.0), logit) for logit in secondary_logits]
        ...
        log_probs = [logit - log_Z_primary.view(-1, 1) for logit in corr_logits_primary]
    ...
    def _check_if_stuck(self):
        ...
        Checks if there is at least one batch element with no action allowed (would cause errors in the loss by indexing on -inf logits)
        """
        x = self.get_primary_logits()
        max_per_type = [
            torch.max(tensor, dim=1) for tensor in x
        ]  # for each graph in batch and for each action type, get max value and index
        max_values_per_type = [pair[0] for pair in max_per_type]
        max_type_logit_values, type_indices = torch.max(torch.stack(max_values_per_type), dim=0)
        at_least_one_batch_element_with_no_action_allowed = torch.any(torch.isinf(max_type_logit_values))
        n_batch_elements_with_no_action_allowed = torch.sum(torch.isinf(max_type_logit_values))
        n_total_batch_elements = len(max_type_logit_values)
        return (
            at_least_one_batch_element_with_no_action_allowed,
            n_batch_elements_with_no_action_allowed,
            n_total_batch_elements,
        )
    ...
    def sample(
        ...
        if random_action_prob > 0.0:
            ...
            is_random_action = torch.tensor(
                rng.uniform(size=self.graphs.num_graphs) < random_action_prob,
                device=device,
            ).bool()
        ...
        for i, t in enumerate(argmax):
            if self.ctx.aidx_to_action_type(t, fwd=self.fwd) == GraphActionType.AddFirstReactant:
                ...
                add_reactant_masks = torch.tensor(
                    self.ctx.create_masks_for_bb_from_precomputed(nx_graphs[i], t[1]),
                    device=device,
                )
        ...
        if self.fwd:
            for i, action in enumerate(actions):
                ...
                if self.ctx.aidx_to_action_type(action, fwd=self.fwd) == GraphActionType.ReactBi:
                    # we compute the AddReactant logits and masks conditionally to the sampled reaction
                    add_reactant_masks = torch.tensor(
                        self.ctx.create_masks_for_bb_from_precomputed(nx_graphs[i], row_idx),
                        device=self.dev,
                    )
                    add_reactant_logits = self.get_addreactant_logits(model, row_idx, self.graph_embeddings[i], self.graphs[i])

                    self._action_masks[self.ctx.action_type_to_aidx(GraphActionType.AddReactant)][i] = add_reactant_masks
    ...
    def logsumexp(
        ...
        if logits is None:
            ...
            primary_logits = [logits[self.action_type_to_logits_index[t]] for t in self.action_hierarchy["fwd"]["primary"]]
            secondary_logits = [logits[self.action_type_to_logits_index[t]] for t in self.action_hierarchy["fwd"]["secondary"]]
