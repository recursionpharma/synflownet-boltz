...
class GraphActionCategorical:
    def __init__(
        ...
        self.slice = [(slice_dict[k].to(dev) if k is not None else torch.arange(graphs.num_graphs + 1, device=dev)) for k in keys]
        self.logprobs = None
        self.log_n = None
    ...
    def logsoftmax(self):
        ...
        logZ = sum([scatter(i, b, dim=0, dim_size=self.num_graphs, reduce="sum").sum(1) for i, b in zip(exp_logits, self.batch)]).log()
    ...
    def logsumexp(self, x=None):
        ...
        reduction = sum([scatter(i, b, dim=0, dim_size=self.num_graphs, reduce="sum").sum(1) for i, b in zip(exp_vals, self.batch)]).log()
        # Add back max
    ...
    def entropy(self, logprobs=None):
        ...
        if logprobs is None:
            logprobs = self.logsoftmax()
        entropy = -sum(
            [scatter(i * i.exp(), b, dim=0, dim_size=self.num_graphs, reduce="sum").sum(1) for i, b in zip(logprobs, self.batch)]
        )
        return entropy
